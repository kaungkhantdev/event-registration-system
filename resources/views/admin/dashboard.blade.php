...
<div class="bg-gray-50 min-h-screen">
    <div class="max-w-7xl mx-auto py-10 px-4 sm:px-6 lg:px-8">
        ...
        <!-- Quick Actions -->
        <div class="bg-white rounded-2xl shadow-xs p-6 mb-8">
            <h2 class="text-xl font-bold text-gray-900 mb-4">Quick Actions</h2>
            <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
                <a href="{{ route('admin.events.create') }}" 
                class="flex items-center justify-center bg-indigo-600 hover:bg-indigo-700 text-white font-medium py-3 px-4 rounded-full shadow-xs transition-colors">
                    <i class="fas fa-plus mr-2"></i> Create Event
                </a>
                <a href="{{ route('admin.events.index') }}" 
                class="flex items-center justify-center bg-gray-600 hover:bg-gray-700 text-white font-medium py-3 px-4 rounded-full shadow-xs transition-colors">
                    <i class="fas fa-list mr-2"></i> Manage Events
                </a>
                <a href="{{ route('admin.registrations.index') }}" 
                class="flex items-center justify-center bg-purple-600 hover:bg-purple-700 text-white font-medium py-3 px-4 rounded-full shadow-xs transition-colors">
                    <i class="fas fa-users mr-2"></i> View Registrations
                </a>
                <a href="{{ route('admin.registrations.index', ['status' => 'pending']) }}" 
                class="flex items-center justify-center bg-yellow-600 hover:bg-yellow-700 text-white font-medium py-3 px-4 rounded-full shadow-xs transition-colors">
                    ...
                    @else
                        <div class="space-y-4">
                            @foreach($recentEvents as $event)
                                <div class="flex justify-between items-center p-4 bg-gray-50 rounded-2xl shadow-xs">
                                    <div>
